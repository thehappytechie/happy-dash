...
<x-print>
    <div class="container padding-top-md cta__button-section">
        <a href="{{ route('shipFishingVessel') }}" class="padding-right-md">Go back</a>
    ...
    <div class="grid justify-between width-90% margin-auto margin-y-xl">
        ...
        <p class="text-center text-sm">FISHING VESSELS</p>
        <div class="margin-top-lg padding-x-xxl">
            <div class="grid gap-xs margin-bottom-sm text-xs">
                <div class="col">IMO number <span>{{ $shipFishingVessel->imo_number }}</span></div>
                <div class="col">
                    <p class="float-right">Signal letters <span>{{ $shipFishingVessel->signal_letter }}</span></p>
                </div>
            </div>
            <table class="text-xs">
                <tr>
                ...
                <tr>
                    <td><span>{{ $shipFishingVessel->ship_name }}</span></td>
                    <td><span>{{ $shipFishingVessel->registry_port }}</span></td>
                    <td><span>{{ $shipFishingVessel->register_tonnage }}</span></td>
                    <td><span>{{ $shipFishingVessel->official_number }}</span></td>
                </tr>
            </table>
            ...
            <div class="flex justify-start margin-top-lg text-sm line-height-lg">
                <div>
                    <p>Dated <span>{{ $shipFishingVessel->issue_date }}</span><br>
                        Port of <span>{{ $shipFishingVessel->location }}</span></p>
                </div>
            </div>
