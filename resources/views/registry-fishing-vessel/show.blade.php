...
<x-print>
    <div class="container padding-top-sm cta__button-section">
        <a href="{{ route('certificateRegistryFishingVessel') }}" class="padding-right-md">Go back</a>
    ...
    <div class="grid justify-between width-90% margin-auto margin-y-lg">
        <x-print-logo>CERTIFICATE OF REGISTRY (FISHING VESSEL)</x-print-logo>
        <div class="margin-top-md padding-x-sm">
            <div class="grid gap-xs margin-bottom-xs text-xs">
                <div class="col">IMO number <span>{{ $registryFishingVessel->imo_number }}</span></div>
                <div class="col font-bold">PARTICULARS OF SHIP</div>
                <div class="col">
                    <p class="float-right">Signal letters <span>{{ $registryFishingVessel->signal_letter }}</span></p>
                </div>
            </div>
            <table class="text-xs">
                <tr>
                ...
                <tr>
                    <td><span>{{ $registryFishingVessel->ship_name }}</span></td>
                    <td><span>{{ $registryFishingVessel->official_number }}</span></td>
                    <td></td>
                    <td></td>
                </tr>
                <tr>
                ...
                <tr>
                    <td><span>{{ $registryFishingVessel->steam_motor }}</span></td>
                    <td><span>{{ $registryFishingVessel->where_built }}</span></td>
                    <td><span>{{ $registryFishingVessel->when_built }}</span></td>
                    <td><span>{{ $registryFishingVessel->name_address_builders }}</span></td>
                </tr>
            </table>
            <table class="text-xs">
                <tr>
                ...
                <tr>
                    <td><span>{{ $registryFishingVessel->decks }}</span></td>
                    <td><span>{{ $registryFishingVessel->masts }}</span></td>
                    <td><span>{{ $registryFishingVessel->rigged }}</span></td>
                    <td><span>{{ $registryFishingVessel->stem }}</span></td>
                    <td><span>{{ $registryFishingVessel->stern }}</span></td>
                    <td><span>{{ $registryFishingVessel->build }}</span></td>
                    <td><span>{{ $registryFishingVessel->vessel_desc }}</span></td>
                    <td><span>{{ $registryFishingVessel->bulkheads }}</span></td>
                </tr>
            </table>
            <table class="text-xs">
                <tr>
                ...
                <tr>
                    <td><span>{{ $registryFishingVessel->overall_length }}</span></td>
                    <td><span>{{ $registryFishingVessel->breadth_mould}}</span></td>
                    <td><span>{{ $registryFishingVessel->depth_mould }}</span></td>
                    <td><span>{{ $registryFishingVessel->engine_length }}</span></td>
                </tr>
            </table>
            <table class="text-xs">
            ...
            <table class="text-xs">
                ...
                <tr>
                    <td>Gross tonnage <span>{{ $registryFishingVessel->gross_ton }}</span></td>
                    <td>Tons(cubic metres) <span>{{ $registryFishingVessel->gross_ton_cubic }}</span></td>
                    <td>Register tonnage <span>{{ $registryFishingVessel->register_ton }}</span></td>
                    <td>Tons(cubic metres) <span>{{ $registryFishingVessel->register_ton_cubic }}</span></td>
                </tr>
                <tr>
                    <td colspan="4">The number of seafarers and apprentices for whom accommodation is certified <span>{{
                            $registryFishingVessel->number_seafarers }}</span>
                    </td>
                </tr>
                <tr>
                    <td><span>{{ $registryFishingVessel->breadth_mould}}</span></td>
                ...
                <tr>
                    <td colspan="2"> <span>{{ $registryFishingVessel->name_residence_owners }}</span></td>
                    <td colspan="2"> <span>{{ $registryFishingVessel->sixty_four_shares }}</span></td>
                </tr>
                <tr>
                    <td colspan="4">
                        <ul class="grid gap-xs">
                            <li class="col">Issued at <span>{{ $registryFishingVessel->location }}</span></li>
                            <li class="col">Date: <span>{{ $registryFishingVessel->issue_date->toFormattedDateString()
                                    }}</span></li>
                            <li class="col-5">
