...
<x-print>
    <div class="container padding-top-md cta__button-section">
        <a href="{{ route('shipInlandWater') }}" class="padding-right-md">Go back</a>
    ...
    <div class="grid justify-between width-90% margin-auto margin-y-xl">
        ...
        <p class="text-center text-sm">INLAND WATER VESSELS (OVER 150 GROSS TONNAGE)</p>
        <div class="margin-top-lg padding-x-xxl">
            <div class="grid gap-xs margin-bottom-sm text-xs">
                <div class="col">IMO number <span>{{ $shipInlandWater->imo_number }}</span></div>
                <div class="col">
                    <p class="float-right">Signal letters <span>{{ $shipInlandWater->signal_letter }}</span></p>
                </div>
            </div>
            <table class="text-xs">
                <tr>
                ...
                <tr>
                    <td><span>{{ $shipInlandWater->ship_name }}</span></td>
                    <td><span>{{ $shipInlandWater->registry_port }}</span></td>
                    <td><span>{{ $shipInlandWater->register_tonnage }}</span></td>
                    <td><span>{{ $shipInlandWater->official_number }}</span></td>
                </tr>
            </table>
            ...
            <div class="flex justify-start margin-top-lg text-sm line-height-lg">
                <div>
                    <p>Dated <span>{{ $shipInlandWater->issue_date }}</span><br>
                        Port of <span>{{ $shipInlandWater->location }}</span></p>
                </div>
            </div>
