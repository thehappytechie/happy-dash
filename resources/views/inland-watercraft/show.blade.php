@section('title', 'Printto Form')

<x-print>
    <div class="container padding-top-md cta__button-section">
        <a href="{{ route('inlandWatercraft') }}" class="padding-right-md">Go back</a>
    ...
    <div class="grid justify-between width-90% margin-auto margin-y-xl">
        ...
        <p class="text-center text-sm">INLAND WATER CRAFT AND LESS THAN TWENTY FOUR (24M) METRES IN LENGTH AND LESS THAN
            150 GROSS TONNAGE
        </p>
        <div class="margin-top-lg padding-x-xxl">
            <div class="grid gap-xs margin-bottom-sm text-xs">
                <div class="col">IMO number <span>{{ $inlandWatercraft->imo_number }}</span></div>
                <div class="col">
                    <p class="float-right">Signal letters <span>{{ $inlandWatercraft->signal_letter }}</span></p>
                </div>
            </div>
            <table class="text-xs">
                <tr>
                ...
                <tr>
                    <td><span>{{ $inlandWatercraft->craft_name }}</span></td>
                    <td><span>{{ $inlandWatercraft->registry_port }}</span></td>
                    <td><span>{{ $inlandWatercraft->register_tonnage }}</span></td>
                    <td><span>{{ $inlandWatercraft->official_number }}</span></td>
                </tr>
            </table>
            ...
            <div class="flex justify-start margin-top-lg text-sm line-height-lg">
                <div>
                    <p>Dated <span>{{ $inlandWatercraft->issue_date }}</span><br>
                        Port of <span>{{ $inlandWatercraft->port_name }}</span></p>
                </div>
            </div>
            ...
            <div class="flex justify-start margin-top-lg text-sm line-height-lg">
                <div>
                    <p>Name of owner <span>{{ $inlandWatercraft->owner }}</span><br>
                        Address of owner <span>{{ $inlandWatercraft->owner_address }}</span></p>
                </div>
            </div>
